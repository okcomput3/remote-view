...
/*
wf::geometry_t add_offset_to_workspace_rect(const wf::geometry_t& workspace_rect, int offset_x, int offset_y)
{
    wf::geometry_t result = workspace_rect;
    result.x += offset_x;
    result.y += offset_y;
    return result;
}*//*
wf::geometry_t subtract_offset_from_workspace_rect(const wf::geometry_t& workspace_rect, int offset_x, int offset_y)
{
    wf::geometry_t result = workspace_rect;
    result.x -= offset_x;
    result.y -= offset_y;
...
class workspace_wall_t : public wf::signal::provider_t
{
  public:
    ...
    wf::geometry_t get_workspace_rectangle(const wf::point_t& ws) const
    {
        ...
        return {
            ws.x * (size.width + gap_size+120000),
